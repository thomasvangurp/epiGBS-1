...
Helpful hints: make sure the barcode is formatted properly; no extra enter at the end -- look at editor with hidden characters as these can be added automatically
...
def parse_bc(barcodes, fc, ln):
    ...
    for line in file_in.readlines():
        if line.startswith("#") or line.startswith("Flowcell"):
            print "Line starts with Flowcell" #MHR debugging
            for n, item in enumerate(line.rstrip('\n').split('\t')):
                header_index[n] = item
        else:
            print "Line starts with else." #MHR debugging
            bc_instance = Barcode()
            for n, item in enumerate(line.rstrip('\n').split('\t')):
            ...
            print len(bc_dict)  # MHR debugging: bc_dict has extra barcode with Nonetype that is interfering with code later on --check and fix
...
def parse_seq_pe(opts, bc_dict, Flowcell, Lane):
    ...
    if not opts.split:
        print "File name is {0}_{1}_s_{2}_fastq.txt".format('R1_%s'%opts.output.split('/'), Flowcell, Lane)
        seq1_name = '%(code)s_%(Flowcell)s_s_%(lane)s_fastq.txt'% \
                    ({'code': 'R1_%s'%opts.output.split('/'),'Flowcell':Flowcell, 'lane':Lane})
        seq2_name = '%(code)s_%(Flowcell)s_s_%(lane)s_fastq.txt'% \
                    ({'code': 'R2_%s'%opts.output.split('/'),'Flowcell':Flowcell, 'lane':Lane})
        if opts.reads1.endswith('.gz'):
            seq1_name += '.gz'
            seq2_name += '.gz'
